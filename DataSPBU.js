            ...
            .map(
              (spbu) => `
                var marker = L.marker([${spbu.Latitude}, ${spbu.Longitude}])
                  .bindPopup('<b>${spbu.Nama || spbu["Nama SPBU"]}</b><br>' +
                    '${spbu.Alamat || "Tidak Tersedia"}<br>' +
                    'Jam Buka: ${spbu.JamBuka || "Buka 24 Jam"}<br>' +
                    'Jam Tutup: ${spbu.JamTutup || "Buka 24 Jam"}<br>' +
                    '<button onclick="openInGoogleMaps(${spbu.Latitude}, ${spbu.Longitude})">Open in Google Maps</button>');
